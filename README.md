...
**Поиск ближайших соседей для точки** на плоскости или в пространстве (используется k-мерное дерево) **и вычисление её значения по ним интерполяцией с обратным взвешиванием расстояний** (метод Шепарда).
...
Проект разрабатывался под стандарт `C++20`, но по факту из него используется только `requires clauses` в шаблонном классе `Point` и пару раз атрибут `[[unlikely]]` в реализациях метода Шепарда, поэтому понизить требование до `C++17` не составит проблем, если это нужно. Была попытка предоставить возможность сборки под стандарт `C++11` с помощью директив препроцессора (условной компиляции) в том же классе `Point`, но найти объективных причин для этого я не смог и поэтому не стал продолжать.
...
Экземпляры класса `KdTree` некопируемые, но перемещаемые. Это можно легко изменить, добавив реализацию глубокого копирования дерева, т.е. его обход с созданием копий умных указателей на узлы (операции копирования по умолчанию, т.е. конструктор копирования и оператор присваивания копированием, создаваемые компилятором, сделают лишь копию умного указателя на корень).
...
Перед запуском нужно подготовить два набора точек (опорных и искомых), желательно <ins>уникальных</ins> по указанным выше причинам (при чтении файлов в функции `readPoints()` это не проверяется), например, с помощью написанного на языке Python генератора `point_generator.py` из этого же репозитория, не забыв добавить в него новую координату, если нужно.
