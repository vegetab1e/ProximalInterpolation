...
public:
    ...
    Point(const C (&coords)[M], V value) noexcept;
...
// было бы использовать 201402L, однако в одном из конструкторов
    ...
    : value_(value)
{
    std::memcpy(coords_,
                coords,
                std::min(sizeof(coords_), sizeof(coords)));
    ...
    : value_(value)
{
    ...
    std::memcpy(coords_,
                coords.begin(),
                std::min(sizeof(coords_), sizeof(*coords.begin()) * coords.size()));
    ...
    : value_(value)
{
    ...
    for (auto j = coords.begin(); i < N && j < coords.end(); ++i, ++j)
        if (std::is_unsigned<C>::value && std::is_signed<T>::value)
            coords_[i] = static_cast<C>(*j < 0 ? negateValue<T>(*j) : *j);
        else
