                                                ...
                                                bool reverse_search) const
{
    ...
    if (reverse_search)
        ...
        directSearchNeighbors(item, root_.get(), 0, max_neighbors, neighbors);
                                         ...
                                         PriorityQueue& neighbors) const
{
    ...
    directSearchNeighbors(item, next_node, depth + 1, max_neighbors, neighbors);

    if (neighbors.size() < max_neighbors)
    {
        directSearchNeighbors(item, auxiliary_node, depth + 1, max_neighbors, neighbors);
    }
    else
    {
                                          ...
                                          PriorityQueue& neighbors) const
{
    ...
    reverseSearchNeighbors(item, next_node, depth + 1, max_neighbors, neighbors);
    ...
    if (neighbors.size() < max_neighbors)
    {
        reverseSearchNeighbors(item, auxiliary_node, depth + 1, max_neighbors, neighbors);
    }
    else
    {
        const auto distance = static_cast<decltype(neighbors.top().first)>(item.getDistance(node->item_, dimension));
