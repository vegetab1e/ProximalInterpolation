...
private:
    ...
    void insertItem(std::shared_ptr<Node>& node,
                    Item&& item,
                    std::size_t depth
#ifndef ALLOW_DUPLICATE_POINTS
                    , bool update = false
    ...
    void searchMinNode(const std::shared_ptr<Node>& node,
                       std::shared_ptr<Node>& min_node,
                       std::size_t depth) const;

    void removeItem(std::shared_ptr<Node>& node,
                    const Item& item,
                    std::size_t depth);

    void searchNeighbors(const Item& item,
                         const Node* node,
                         std::size_t depth,
                         std::size_t max_neighbors,
...
std::vector<Item> KdTree<Item>::searchNeighbors(const Item& item,
                                                ...
                                                bool reverse_search) const
{
    if (!root_ || !max_neighbors)
        return {};

    Container container;
    container.reserve(max_neighbors);

    PriorityQueue neighbors{CompareLess(), std::move(container)};
    if (reverse_search)
        reverseSearchNeighbors(item, root_.get(), 0, max_neighbors, neighbors);
    else
        searchNeighbors(item, root_.get(), 0, max_neighbors, neighbors);
...
#else
        const auto weight = 1.0 / std::pow(std::max(neighbor.first, EPSILON), idw_power);
#endif
                                                                     ...
                                                                     std::size_t depth) const
{
    ...
    std::sort(items.begin(), items.end(),
              [dim = depth % num_dims_](const auto& lhs, const auto& rhs)->bool{return lhs.compareLess(rhs, dim);});
...
template<class Item>
void KdTree<Item>::insertItem(std::shared_ptr<Node>& node,
                              Item&& item,
                              std::size_t depth
#ifndef ALLOW_DUPLICATE_POINTS
                              , bool update
...
)
{
    ...
    if (item.compareLess(node->item_, depth % num_dims_))
        insertItem(node->left_, std::move(item), depth + 1);
    else
        insertItem(node->right_, std::move(item), depth + 1);
}

template<class Item>
void KdTree<Item>::searchMinNode(const std::shared_ptr<Node>& node,
                                 std::shared_ptr<Node>& min_node,
                                 std::size_t depth) const
{
    if (node->item_.compareLess(min_node->item_, depth % num_dims_))
        min_node = node;

    if (node->left_)
        searchMinNode(node->left_, min_node, depth + 1);

    if (node->right_)
        searchMinNode(node->right_, min_node, depth + 1);
}

template<class Item>
void KdTree<Item>::removeItem(std::shared_ptr<Node>& node,
                              const Item& item,
                              std::size_t depth)
{
    if (!node)
    ...
    if (item.compareEqual(node->item_))
    {
        ...
        else
        {
            ...
            node->item_ = min_node->item_;

            removeItem(min_node, min_node->item_, depth + 1);
        }
    ...
    if (item.compareLess(node->item_, depth % num_dims_))
        removeItem(node->left_, item, depth + 1);
    else
        removeItem(node->right_, item, depth + 1);
}

template<class Item>
void KdTree<Item>::searchNeighbors(const Item& item,
                                   const Node* node,
                                   std::size_t depth,
                                   std::size_t max_neighbors,
                                   PriorityQueue& neighbors) const
{
    ...
    const auto dimension = depth % num_dims_;
    ...
    searchNeighbors(item, next_node, depth + 1, max_neighbors, neighbors);

    if (neighbors.size() < max_neighbors)
    {
        searchNeighbors(item, auxiliary_node, depth + 1, max_neighbors, neighbors);
    }
    else
    {
